...
use crate::{BUILTIN_EXAMPLE, graph, parse, render};
...
pub fn run(parent_id: &str) -> Result<(), JsValue> {
    ...
    let program = parse(BUILTIN_EXAMPLE).map_err(|e| to_js(e.to_string()))?;
    ...
    render::render(&svg, &graph).map_err(|e| to_js(format!("{e:?}")))?;
